...
crossPriceABA.long$pred <- tn$inverse(predict(out))
...
Model comparisons revealed that the 3-parameter form of the ZBE model better characterized own-price demand for EBPs than the two-parameter (*F[`r df1 - df2`, `r df2`]*=`r round(Fval,2)`, `r scales::pvalue(pval, add_p = TRUE)`) and one-parameter alternatives (*F[`r df1_2 - df2_2`, `r df2_2`]*=`r round(Fval_2,2)`, `r scales::pvalue(pval_2, add_p = TRUE)`). The 3-parameter form of the ZBE model was used to estimate *Q~0~*, $\alpha$, and *k* parameters in the same manner as in the Alone-Price demand for EBPs. The results of this regression are listed in Table XXXX and displayed in Figure 2. Model fits indicated that men demonstrated higher baseline levels of EBP consumption than women when an alternative therapy was available (*Q~0~ [Male]* = `r coef(summary(out))["q0.SexMale", "Value"]`, *T* = `r coef(summary(out))["q0.SexMale", "t-value"]`, *p* \< .05). However, men demonstrated greater sensitivity to changes in prices than women ($\alpha$ *[Male]* = `r coef(summary(out))["alpha.SexMale", "Value"]`, *T* = `r coef(summary(out))["alpha.SexMale", "t-value"]`, *p* \< .0001). Further, results indicated that caregivers of a single child demonstrated greater baseline levels of EBP consumption than those who cared for multiple children (*Q~0~ [Single]* = `r coef(summary(out))["q0.FamilySizeSingle", "Value"]`, *T* = `r coef(summary(out))["q0.FamilySizeSingle", "t-value"]`, *p* \< .01). Population-level predictions revealed a peak expenditure (*O~MAX~*) of 1,719.3 USD towards EBPs, which occurred at a price (*P~MAX~*) of 522.65 USD per unit hour of therapy.
...
```{r rqxtable, results='asis', eval=FALSE}

texreg(out, caption = "Modeled Demand for Evidence-based Practices")
