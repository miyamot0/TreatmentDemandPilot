...
\begin{tabular}{lllll}
\multicolumn{5}{c}{{\ul \textbf{Participant Demographics (n = 54)}}}                                                                             \\ \hline
\multicolumn{2}{l}{{\ul \textbf{Age (years)}}}      & \multicolumn{1}{l|}{{\ul \textbf{}}} & {\ul \textbf{Number of Children}} & {\ul \textbf{}} \\
\multicolumn{2}{l}{Mean (SD)}                       & \multicolumn{1}{l|}{38.7 (9.6)}      & Median (Q1-Q3)                    & 2 (1-2)         \\
\multicolumn{2}{l}{Median (Q1-Q3)}                  & \multicolumn{1}{l|}{38 (30.2-44.8)}  & Mean (SD)                         & 1.85 (0.96)     \\
\multicolumn{2}{l}{{\ul \textbf{Sex}}}              & \multicolumn{1}{l|}{{\ul \textbf{}}} & {\ul \textbf{Education}}          &                 \\
\multicolumn{2}{l}{Male}                            & \multicolumn{1}{l|}{28 (51.9\%)}     & High School graduate              & 11 (20.4\%)     \\
\multicolumn{2}{l}{Female}                          & \multicolumn{1}{l|}{26 (48.1\%)}     & Some college but no degree        & 8 (14.8\%)      \\
\multicolumn{2}{l}{{\ul \textbf{Income}}}           & \multicolumn{1}{l|}{{\ul \textbf{}}} & Associate degree                  & 10 (18.5\%)     \\
\multicolumn{2}{l}{Q1}                              & \multicolumn{1}{l|}{30,250 USD}      & Bachelor’s degree                 & 21 (38.9\%)     \\
\multicolumn{2}{l}{Median}                          & \multicolumn{1}{l|}{50,000 USD}      & Master’s degree                   & 4 (7.4\%)       \\
\multicolumn{2}{l}{Q3}                              & \multicolumn{1}{l|}{82,500 USD}      & {\ul \textbf{Race/Ethnicity}}     &                 \\
\multicolumn{2}{l}{{\ul \textbf{Behavior Concern}}} & \multicolumn{1}{l|}{}                & African-American                  & 3 (5.5\%)       \\
\multicolumn{2}{l}{A little}                        & \multicolumn{1}{l|}{26 (48.1\%)}     & Asian                             & 7 (13.0\%)      \\
\multicolumn{2}{l}{A moderate amount}               & \multicolumn{1}{l|}{11 (20.4\%)}     & Hispanic/Latinx                   & 1 (1.8\%)       \\
\multicolumn{2}{l}{A lot}                           & \multicolumn{1}{l|}{12 (22.2\%)}     & White/Caucasian                   & 42 (77.8\%)     \\
\multicolumn{2}{l}{A great deal}                    & \multicolumn{1}{l|}{5 (9.2\%)}       & Native American                   & 1 (1.8\%)       \\
\multicolumn{2}{l}{{\ul \textbf{Marital Status}}}   & \multicolumn{1}{l|}{{\ul \textbf{}}} &                                   &                 \\
\multicolumn{2}{l}{Single}                          & \multicolumn{1}{l|}{12 (22.2\%)}     &                                   &                 \\
\multicolumn{2}{l}{Married}                         & \multicolumn{1}{l|}{39 (72.2\%)}     & \multicolumn{2}{l}{}                                \\
\multicolumn{2}{l}{Divorced}                        & \multicolumn{1}{l|}{3 (5.5\%)}       & \multicolumn{2}{l}{}                               
\end{tabular}
\label{tab:demos}
\end{table}
...
```{r rq2plot, fig.cap="Own-Price Demand for Evidence-based Practices", fig.scap="Plots depicted here show the Own-Price demand for Evidence-based Practices across gender, family size, and degree of education.", fig.height=4.5, fig.width=8.5}
...
Model comparisons revealed that the 3-parameter form of the ZBE model better characterized own-price demand for EBPs than the two-parameter (*F[`r df1 - df2`, `r df2`]*=`r round(Fval,2)`, `r scales::pvalue(pval, add_p = TRUE)`) and one-parameter alternatives (*F[`r df1_2 - df2_2`, `r df2_2`]*=`r round(Fval_2,2)`, `r scales::pvalue(pval_2, add_p = TRUE)`). The 3-parameter form of the ZBE model was used to estimate *Q~0~*, $\alpha$, and *k* parameters in the same manner as in the Alone-Price demand for EBPs. The results of this regression are listed in \autoref{tab:models} and displayed in \autoref{fig:rq2plot}. Model fits indicated that men demonstrated higher baseline levels of EBP consumption than women when an alternative therapy was available (*Q~0~ [Male]* = `r coef(summary(out2))["q0.SexMale", "Value"]`, *T* = `r coef(summary(out2))["q0.SexMale", "t-value"]`, `r coef(summary(out2))["q0.SexMale", "p-value"]`). However, men demonstrated greater sensitivity to changes in prices than women ($\alpha$ *[Male]* = `r coef(summary(out2))["alpha.SexMale", "Value"]`, *T* = `r coef(summary(out2))["alpha.SexMale", "t-value"]`, `r coef(summary(out2))["alpha.SexMale", "p-value"]`). Further, results indicated that caregivers of a single child demonstrated greater baseline levels of EBP consumption than those who cared for multiple children (*Q~0~ [Single]* = `r coef(summary(out2))["q0.FamilySizeSingle", "Value"]`, *T* = `r coef(summary(out2))["q0.FamilySizeSingle", "t-value"]`, `r coef(summary(out2))["q0.FamilySizeSingle", "p-value"]`). Population-level predictions revealed a peak expenditure (*O~MAX~*) of `r round(which.max(dataProj$Work), 2)` USD towards EBPs, which occurred at a price (*P~MAX~*) of `r round(pMax[1], 2)` USD per unit hour of therapy.
...
The GEE was applied using the *geeglm* method included in the *geepack* R package [@RN5183]. Factors in the GEE fitting included Price (of EBP), Gender (Men, Women), Family Size (Single, Multiple Children), and Education (i.e., No College, \<= 2 Yr College, \>= 4 Yr College) and all possible interactions. Model selection using QIC favored the model with Price as the sole factor associated with the consumption of ATs ($\alpha$ [\*Price]\* = `r coef(summary(fit.6))["Price", "Estimate"]`, *W* = `r round(coef(summary(fit.6))["Price", "Estimate"], 2)`, `r scales::pvalue(coef(summary(fit.6))["Price", "Pr(>|W|)"], add_p = TRUE)`. Overall, results indicated that caregivers substituted ATs for EBPs as the price to consume EBPs increased. No other demographic factors were significantly related to levels of AT consumption.
